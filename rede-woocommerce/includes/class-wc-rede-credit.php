...
class WC_Rede_Credit extends WC_Rede_Abstract {
	...
	public function __construct() {
		...
		$this->method_title = esc_attr__( 'Pay with the Rede', 'rede-woocommerce' );
		$this->method_description = esc_attr__( 'Enables and configures payments with Rede', 'rede-woocommerce' );
		$this->supports = array(
			'products',
			'refunds',
		);
	...
	public function display_meta( $order ) {
		?>
		<h3><?php esc_attr_e( 'Rede', 'rede-woocommerce' ); ?></h3>
		<table>
			<tbody>
			<tr>
				<td><?php esc_attr_e( 'Environment', 'rede-woocommerce' ); ?></td>
				<td><?php echo $order->get_meta( '_wc_rede_transaction_environment' ); ?></td>
			</tr>

			<tr>
				<td><?php esc_attr_e( 'Return Code', 'rede-woocommerce' ); ?></td>
				<td><?php echo $order->get_meta( '_wc_rede_transaction_return_code' ); ?></td>
			</tr>

			<tr>
				<td><?php esc_attr_e( 'Return Message', 'rede-woocommerce' ); ?></td>
				<td><?php echo $order->get_meta( '_wc_rede_transaction_return_message' ); ?></td>
			</tr>

			<?php if ( ! empty( $order->get_meta( '_wc_rede_transaction_id' ) ) ) { ?>
				<tr>
					<td><?php esc_attr_e( 'Transaction ID', 'rede-woocommerce' ); ?></td>
			...
			<?php if ( ! empty( $order->get_meta( '_wc_rede_transaction_refund_id' ) ) ) { ?>
				<tr>
					<td><?php esc_attr_e( 'Refund ID', 'rede-woocommerce' ); ?></td>
			...
			<?php if ( ! empty( $order->get_meta( '_wc_rede_transaction_cancel_id' ) ) ) { ?>
				<tr>
					<td><?php esc_attr_e( 'Cancellation ID', 'rede-woocommerce' ); ?></td>
			...
			<?php if ( ! empty( $order->get_meta( '_wc_rede_transaction_nsu' ) ) ) { ?>
				<tr>
					<td><?php esc_attr_e( 'Nsu', 'rede-woocommerce' ); ?></td>
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				'title' => esc_attr__( 'Enable/Disable', 'rede-woocommerce' ),
				'type' => 'checkbox',
				'label' => esc_attr__( 'Enables payment with Rede', 'rede-woocommerce' ),
				'default' => 'yes',
			),
			'title' => array(
				'title' => esc_attr__( 'Title', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => esc_attr__( 'Pay with Rede', 'rede-woocommerce' ),
			),

			'rede' => array(
				'title' => esc_attr__( 'General configuration', 'rede-woocommerce' ),
				'type' => 'title',
			),
			'environment' => array(
				'title' => esc_attr__( 'Environment', 'rede-woocommerce' ),
				'type' => 'select',
				'description' => esc_attr__( 'Choose the environment', 'rede-woocommerce' ),
				'desc_tip' => true,
				'class' => 'wc-enhanced-select',
				'default' => esc_attr__( 'test', 'rede-woocommerce' ),
				'options' => array(
					'test' => esc_attr__( 'Tests', 'rede-woocommerce' ),
					'production' => esc_attr__( 'Production', 'rede-woocommerce' ),
				),
			),
			'pv' => array(
				'title' => esc_attr__( 'PV', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => '',
			),
			'token' => array(
				'title' => esc_attr__( 'Token', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => '',
			),

			'soft_descriptor' => array(
				'title' => esc_attr__( 'Soft Descriptor', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => '',
			),

			'credit_options' => array(
				'title' => esc_attr__( 'Credit Card Settings', 'rede-woocommerce' ),
				'type' => 'title',
			),

			'auto_capture' => array(
				'title' => esc_attr__( 'Authorization and Capture', 'rede-woocommerce' ),
				'type' => 'select',
				'class' => 'wc-enhanced-select',
				'default' => '2',
				'options' => array(
					'1' => esc_attr__( 'Authorize and capture automatically', 'rede-woocommerce' ),
					'0' => esc_attr__( 'Just authorize', 'rede-woocommerce' ),
				),
			),
			'min_parcels_value' => array(
				'title' => esc_attr__( 'Value of the smallest installment', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => '0',
			),
			'max_parcels_number' => array(
				'title' => esc_attr__( 'Max installments', 'rede-woocommerce' ),
				'type' => 'select',
				'class' => 'wc-enhanced-select',
				'default' => '12',
				'options' => array(
			...
			'partners' => array(
				'title' => esc_attr__( 'Partner Settings', 'rede-woocommerce' ),
				'type' => 'title',
			),
			'module' => array(
				'title' => esc_attr__( 'Module ID', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => '',
			),
			'gateway' => array(
				'title' => esc_attr__( 'Gateway ID', 'rede-woocommerce' ),
				'type' => 'text',
				'default' => '',
			),

			'developers' => array(
				'title' => esc_attr__( 'Developer Settings', 'rede-woocommerce' ),
				'type' => 'title',
			),

			'debug' => array(
				'title' => esc_attr__( 'Debug', 'rede-woocommerce' ),
				'type' => 'checkbox',
				'label' => esc_attr__( 'Enable debug logs', 'rede-woocommerce' ),
				'default' => esc_attr__( 'no', 'rede-woocommerce' ),
			),
		);
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( empty( $order->get_meta( '_wc_rede_transaction_canceled' ) ) ) {
			...
			try {
				...
				$order->add_order_note( esc_attr_e( 'Refunded:', 'rede-woocommerce' ) . wc_price( $amount ) );
			} catch ( Exception $e ) {
	...
	public function process_capture( $order_id ) {
		...
		if ( empty( $order->get_meta( '_wc_rede_captured' ) ) ) {
			...
			try {
				...
				$order->add_order_note( esc_attr_e( 'Captured', 'rede-woocommerce' ) );
			} catch ( Exception $e ) {
