...
abstract class WC_Rede_Abstract extends WC_Payment_Gateway {
	...
	public function order_items_payment_details( $items, $order ) {
		...
		if ( $this->id === $order->get_payment_method() ) {
			...
			$items['payment_return'] = array(
				'label' => esc_attr__( 'Payment:', 'rede-woocommerce' ),
				'value' => sprintf(
					__( '<strong>Order ID</strong>: %1$s<br /><strong>Installments</strong>: %2$s<br /><strong>Transaction Id</strong>: %3$s<br />', 'rede-woocommerce' ),
	...
	public function consult_order( $order, $id, $tid, $status ) {
		...
		$this->process_order_status( $order, $transaction, esc_attr_e( 'automatic check', 'rede-woocommerce' ) );
	}
	...
	public function process_order_status( $order, $transaction, $note = '' ) {
		...
		if ( $transaction->getReturnCode() == '00' ) {
			if ( $transaction->getCapture() ) {
				...
				$order->update_status( esc_attr_e( 'on-hold', 'rede-woocommerce' ) );
				wc_reduce_stock_levels( $order->get_id() );
			}
		} else {
			$order->update_status( esc_attr_e( 'failed', 'rede-woocommerce' ), $status_note );
			$order->update_status( esc_attr_e( 'cancelled', 'rede-woocommerce' ), $status_note );
		}
	...
	public function thankyou_page( $order_id ) {
		...
		if ( $order->get_status() == esc_attr__( 'on-hold', 'rede-woocommerce' ) || $order->get_status() == esc_attr__( 'processing', 'rede-woocommerce' ) || $order->get_status() == esc_attr__( 'completed', 'rede-woocommerce' ) ) {
			echo '<div class="woocommerce-message">' . esc_attr__( 'Your order is already being processed. For more information ', 'rede-woocommerce' ) . '<a href="' . esc_url( $order_url ) . '" class="button" style="display: block !important; visibility: visible !important;">' . esc_attr__( 'see order details', 'rede-woocommerce' ) . '</a><br /></div>';
		} else {
			echo '<div class="woocommerce-info">' . esc_attr__( 'For more details on your order, please visit ', 'rede-woocommerce' ) . '<a href="' . esc_url( $order_url ) . '">' . esc_attr__( 'order details page', 'rede-woocommerce' ) . '</a></div>';
		}
	}
	...
	protected function validate_card_number( $card_number ) {
		...
		if ( array_sum( str_split( $card_number_checksum ) ) % 10 !== 0 ) {
			throw new Exception( esc_attr_e( 'Please enter a valid credit card number', 'rede-woocommerce' ) );
		}
	...
	protected function validate_card_fields( $posted ) {
		...
		try {
			if ( ! isset( $posted[ $this->id . '_holder_name' ] ) || '' === $posted[ $this->id . '_holder_name' ] ) {
				throw new Exception( esc_attr_e( 'Please enter cardholder name', 'rede-woocommerce' ) );
			}
			...
			if ( preg_replace(
				...
				throw new Exception( esc_attr_e( 'Cardholder name can only contain letters', 'rede-woocommerce' ) );
			}

			if ( ! isset( $posted[ $this->id . '_expiry' ] ) || '' === $posted[ $this->id . '_expiry' ] ) {
				throw new Exception( esc_attr_e( 'Please enter card expiration date', 'rede-woocommerce' ) );
			}
			...
			if ( isset( $posted[ $this->id . '_expiry' ][2] ) && ! isset( $posted[ $this->id . '_expiry' ][3] ) ) {
				throw new Exception( esc_attr_e( 'Expiration date must contain 2 or 4 digits', 'rede-woocommerce' ) );
			}
			...
			if ( strtotime(
				...
				throw new Exception( esc_attr_e( 'Card expiration date must be future.', 'rede-woocommerce' ) );
			}

			if ( ! isset( $posted[ $this->id . '_cvc' ] ) || '' === $posted[ $this->id . '_cvc' ] ) {
				throw new Exception( esc_attr_e( 'Please enter card security code', 'rede-woocommerce' ) );
			}

			if ( preg_replace( '/[^0-9]/', '', $posted[ $this->id . '_cvc' ] ) != $posted[ $this->id . '_cvc' ] ) {
				throw new Exception( esc_attr_e( 'Security code must contain only numbers', 'rede-woocommerce' ) );
			}
		} catch ( Exception $e ) {
	...
	protected function validate_installments( $posted, $order_total ) {
		...
		try {
			if ( ! isset( $posted['rede_credit_installments'] ) || '' === $posted['rede_credit_installments'] ) {
				throw new Exception( esc_attr_e( 'Please enter the number of installments', 'rede-woocommerce' ) );
			}
			...
			if ( $installments > $max_parcels || ( ( $min_value != 0 ) && ( ( $order_total / $installments ) < $min_value ) ) ) {
				throw new Exception( esc_attr_e( 'Invalid number of installments', 'rede-woocommerce' ) );
			}
		} catch ( Exception $e ) {
			$this->add_error( $e->getMessage() );
